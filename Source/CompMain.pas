...
begin
  Result := (lex.tokL ='var') or (lex.tokL ='const') or
  (lex.tokL ='type') or (lex.tokL ='procedure') or (lex.tokL ='inline');
end;
...
begin
  ...
  if lex.tokL <> 'end' then begin  //verifica si termina el programa
...
{  Op.catTyp := t_string;   //es cadena
  ...
  //toma el texto
  Op.valStr := copy(lex.tok,2, length(lex.tok)-2);   //quita comillas
...
var
  ...
  IsBit: boolean;
begin
  aditVar.srcDec  := lex.GetCtxState;  //Posición de inicio de posibles parámetros adic.
  aditVar.hasAdic := decNone;       //Bandera
  aditVar.hasInit := false;
  aditVar.absVar := nil;          //Por defecto
  if (lex.tokL = 'absolute') or (lex.token = '@') then begin
    //// Hay especificación de dirección absoluta ////
    aditVar.hasAdic := decAbsol;    //marca bandera
    lex.Next;
    ProcComments;
    if lex.tokType = tnNumber then begin
      if (lex.token[1]<>'$') and ((pos('e', lex.token)<>0) or (pos('E', lex.token)<>0)) then begin
        //La notación exponencial, no es válida.
        GenError(ER_INV_MEMADDR);
        exit;
      end;
      n := pos('.', lex.token);   //no debe fallar
      if n=0 then begin
        //Número entero sin parte decimal
        aditVar.absAddr := ReadAddres(lex.token);
        lex.Next;  //Pasa con o sin error, porque esta rutina es "Pasa siempre."
        //Puede que siga la parte de bit
        if lex.token = '.' then begin
          lex.Next;
          IsBit := true;  //Tiene parte de bit
          aditVar.absBit := ReadAddresBit(lex.token);  //Parte decimal
          lex.Next;  //Pasa con o sin error, porque esta rutina es "Pasa siempre."
        end else begin
        ...
        aditVar.absAddr := ReadAddres(copy(lex.token, 1, n-1));
        //Puede haber error
        aditVar.absBit := ReadAddresBit(copy(lex.token, n+1, 100));  //Parte decimal
        lex.Next;  //Pasa con o sin error, porque esta rutina es "Pasa siempre."
      end;
    end else if lex.tokType = tnIdentif then begin
      //Puede ser variable
  ...
  end else if lex.tokL = 'register' then begin
    // Es de tipo registro
...
en el procesamiento de unidades.}
  ...
  begin
    ...
    if EOBlock or EOExpres or (lex.token = ':') then begin
      ...
      lex.SkipWhites;
      repeat
        //if lex.tokL = 'register' then begin
...
begin
  ...
  lex.SkipWhites;
  if lex.tokL = 'interrupt' then begin
    lex.Next;
    IsInterrupt := true;
    if not CaptureTok(';') then exit;
  end else begin
  ...
  if lex.tokL = 'forward' then begin
    lex.Next;
    IsForward := true;
    if not CaptureTok(';') then exit;
  end else begin
...
//      end;
  ...
  begin
    ...
    reftyp := FindSysEleType(lex.token); //Busca elemento
    if reftyp = nil then begin
      //No es un tipo del sistema, pero puede ser un tipo prdefinido
      reftyp := TreeElems.FindType(lex.token); //Busca elemento
      if reftyp = nil then begin
...
begin
  ...
  try
    ...
    if (lex.tokType = tnType) then begin
    ...
    end else if (lex.tokL = 'array') or (lex.token = '[') then begin
    ...
    end else if {(lex.tokL = 'pointer') or }(lex.token = '^') then begin
...
//  end else if lex.tok = ':' then begin
  end else begin
...
begin
  ...
  if OnRequireFileString<>nil then begin
    ...
    if strList=nil then begin
      ...
      lex.MsjError := '';
      lex.NewContextFromFile(filePath);
      Result := lex.MsjError='';  //El único error es cuando no se encuentra el archivo.
    end else begin
      //Nos están dando el acceso al contenido. Usamos "strList"
      lex.NewContextFromFile(filePath, strList);
      Result := true;
    end;
  end else begin
...
begin
  ...
  if lex.tokL='begin' then begin
    ...
    //es una instrucción
    if lex.tokType = tnStruct then begin
      if lex.tokl = 'if' then begin
        curCodCon.OpenBlock(sbiIF);
        lex.Next;         //pasa "if"
        callCompileIF;
        curCodCon.CloseBlock;
      end else if lex.tokl = 'while' then begin
        curCodCon.OpenBlock(sbiWHILE);
        lex.Next;         //pasa "while"
        callCompileWHILE;
        curCodCon.CloseBlock;
      end else if lex.tokl = 'repeat' then begin
        curCodCon.OpenBlock(sbiREPEAT);
        lex.Next;         //pasa "until"
        callCompileREPEAT;
        curCodCon.CloseBlock;
      end else if lex.tokl = 'for' then begin
...
begin
  ProcComments;
  while not lex.atEof and (lex.tokType<>tnBlkDelim) do begin
    ...
    if lex.tokType=tnBlkDelim then break;
...
begin
  ...
  //Busca UNIT
  if lex.tokL = 'unit' then begin
    lex.curCtx.SetStartPos;   //retorna al inicio
    exit(true);
  end;
  lex.curCtx.SetStartPos;   //retorna al inicio
  exit(false);
...
begin
  ...
  while StartOfSection do begin
    if lex.tokL = 'var' then begin
      lex.Next;    //lo toma
      while not StartOfSection and (lex.tokL <>'begin') do begin
        CompileVarDeclar;
        if HayError then exit;;
      end;
    end else if lex.tokL = 'const' then begin
      lex.Next;    //lo toma
      while not StartOfSection and (lex.tokL <>'begin') do begin
        CompileGlobalConstDeclar;
        if HayError then exit;;
      end;
//    end else if lex.tokL = 'procedure' then begin
//      lex.Next;    //lo toma
//      CompileProcDeclar;
    end else begin
  ...
  fun.posCtx := lex.GetCtxState;  //Guarda posición para la segunda compilación
  bod := CreateBody;   //crea elemento del cuerpo de la función
  bod.srcDec := lex.GetSrcPos;
  ...
  bod.srcEnd := lex.GetSrcPos;  //Fin de cuerpo
...
var
  ...
  fundec: TxpEleFunDec;
begin
//debugln('   Ini Unit: %s-%s',[TreeElems.curNode.name, ExtractFIleName(lex.curCtx.arc)]);
  ClearError;
  ...
  if lex.tokL = 'unit' then begin
    ...
    if UpCase(lex.token)<>UpCase(uni.name) then begin
  ...
  ProcComments;
  if lex.tokL <> 'interface' then begin
...
//  Cod_StartProgram;  //Se pone antes de codificar procedimientos y funciones
  ...
  while StartOfSection do begin
    if lex.tokL = 'var' then begin
      lex.Next;    //lo toma
      while not StartOfSection and (lex.tokL <>'implementation') do begin
        CompileVarDeclar(true);  //marca como "IsInterface"
        if HayError then exit;;
      end;
    end else if lex.tokL = 'type' then begin
      lex.Next;    //lo toma
      while not StartOfSection and (lex.tokL <>'implementation') do begin
  ...
  ProcComments;
  if lex.tokL <> 'implementation' then begin
  ...
  while StartOfSection do begin
    if lex.tokL = 'var' then begin
      lex.Next;    //lo toma
      while not StartOfSection and (lex.tokL <>'end') do begin
        CompileVarDeclar;
        if HayError then exit;;
      end;
    end else if lex.tokL = 'const' then begin
...
//  end;
//  Cod_EndProgram;
//debugln('   Fin Unit: %s-%s',[TreeElems.curNode.name, ExtractFIleName(lex.curCtx.arc)]);
end;
...
//debugln('Abs: xvar=%s at %d', [xvar.name, xvar.adicPar.absAddr]);
    ...
    posAct := lex.GetCtxState;   //guarda posición actual
    lex.SetCtxState(xVar.adicPar.srcDec);  //Posiciona en la declaración adicional
...
begin
  ...
  if lex.tokL = 'program' then begin
    lex.Next;  //pasa al nombre
    ProcComments;
    if lex.atEof then begin
