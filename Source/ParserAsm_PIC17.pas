...
unit ParserAsm_PIC17;
{$mode objfpc}{$H+}
interface
uses
  Classes, SysUtils, fgl, SynFacilHighlighter, Pic17Utils, GenCod_PIC17, Globales,
...
type
  ...
  private
    ...
    procedure AddUJump(name: string; addr: integer; idInst: TPIC17Inst);
    function CaptureAddress(const idInst: TPIC17Inst; var a: word): boolean;
    function CaptureBitVar(out f, b: byte): boolean;
    function CaptureByte(out k: byte): boolean;
    function CaptureComma: boolean;
    function CaptureDestinat(out d: TPIC17destin): boolean;
...
procedure TParserAsm.AddUJump(name: string; addr: integer; idInst: TPIC17Inst);
...
function TParserAsm.CaptureDestinat(out d: TPIC17destin): boolean;
...
function TParserAsm.CaptureAddress(const idInst: TPIC17Inst; var a: word): boolean;
...
begin
  ...
  if upcase(tok) = 'ORG' then begin
    lexAsm.Next;
    idInst := i_GOTO;  //no debería ser necesario
