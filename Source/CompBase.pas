...
protected //Calls to Code Generator (GenCodeBas)
  callCurrFlash   : function(): integer of object;
  callResetFlashAndRAM: procedure of object;
  ...
  callReturnAttribIn  : function(typ: TxpEleType; const Op: TOperand; offs: integer): boolean of object;
...
protected //Calls to Code Generator
  ...
  callDefineArray : procedure(etyp: TxpEleType) of object;
  callDefinePointer : procedure(etyp: TxpEleType) of object;
...
protected //ROP and expressions
  ...
  procedure GetOperand(out Op: Toperand; ToSet: boolean); virtual;
...
var
  ...
  ER_SEMIC_EXPEC , ER_STR_EXPECTED, ER_TYP_PARM_ER_, ER_UNKNOWN_IDE_,
  ER_IN_EXPRESSI , ER_OPERAN_EXPEC, ER_ILLEG_OPERA_, ER_UND_OPER_TY_,
  ER_CAN_AP_OPER_, ER_IN_CHARACTER, ER_INV_COD_CHAR, ER_ONLY_ONE_REG,
  ER_RA_HAV_USED
  : string;
...
begin
  lex.SkipWhites;
  while (lex.tokType = tnDirective) or (lex.tokType = tnAsm) do begin
    if lex.tokType = tnAsm then begin
      //Es una línea ASM
      callProcASMlime(lex.token);  //procesa línea
...
//        lex.SkipWhites;   {En el nuevo contexto puede haber nuevos comentarios.}
...
//function TCompilerBase.AddType(typName: string; srcPos: TSrcPos;
//                               catType: TxpCatType): TxpEleType;
//{Crea un elemento tipo y lo agrega en el nodo actual del árbol de sintaxis.
//Si no hay errores, devuelve la referencia al tipo. En caso contrario, devuelve NIL.
//}
//var
//  xtyp: TxpEleType;
//begin
//  //Inicia parámetros adicionales de declaración
//  xtyp := CreateEleType(typName, srcPos, catType);
//  Result := xtyp;
...
begin
  ...
  repeat
    ...
    if lex.tokType <> tnIdentif then begin
...
begin
  ...
  lex.Next;    //Toma el "."
  if (lex.tokType<>tnIdentif) and (lex.tokType<>tnNumber) then begin
...
//debugln(' --Element ' + lex.tok + ':' + ele.Path);
  if ele.idClass = eltVar then begin
    ...
    decNone, decAbsol: begin
      ...
      if (lex.token = '.') or (lex.token = '[') then begin
    ...
    posCall := lex.GetSrcPos;   //guarda la posición de llamada.
    lex.Next;               //Toma identificador
    lex.SkipWhites;         //Quita posibles blancos
    posPar := lex.GetCtxState;   //Guarda porque va a pasar otra vez por aquí
...
begin
  //lex.SkipWhites;
  ProcComments;
  Op.Inverted := false;   //inicia campo
  if lex.tokType = tnNumber then begin  //constantes numéricas
    {$IFDEF LogExpres} Op.txt:= cIn.tok; {$ENDIF}   //toma el texto
    TipDefecNumber(Op, lex.token); //encuentra tipo de número, tamaño y valor
    if HayError then exit;  //verifica
    lex.Next;    //Pasa al siguiente
  end else if lex.tokType = tnChar then begin  //constante caracter
    {$IFDEF LogExpres} Op.txt:= cIn.tok; {$ENDIF}   //toma el texto
    ...
    Op.valInt := cod;
    lex.Next;    //Pasa al siguiente
  end else if lex.tokType = tnString then begin  //constante cadena
    nElem := length(lex.token) - 2;  //Don't consider quotes
    str := copy(lex.token, 2, nElem);
    lex.Next;    //Pasa al siguiente
    if lex.tokType = tnChar then  begin  //like #255
      //Concat the next char to simulate concat, considering there is not a
      //string type.
      ascCode := StrToInt(Copy(lex.token,2,3));
    ...
    if length(str) = 1 then begin
      ...
      //Op.valStr := copy(lex.tok, 2, length(lex.tok)-2);
    end;
  end else if (lex.tokType = tnSysFunct) or //función del sistema
              (lex.tokL = 'bit') or    //"bit" es de tipo "tnType"
              (lex.tokL = 'boolean') or //"boolean" es de tipo "tnType"
              (lex.tokL = 'byte') or    //"byte" es de tipo "tnType"
              (lex.tokL = 'word') or    //"word" es de tipo "tnType"
              (lex.tokL = 'dword') then begin  //"dword" es de tipo "tnType"
    {Se sabe que es función, pero no se tiene la función exacta porque puede haber
     versiones, sobrecargadas de la misma función.}
    tmp := UpCase(lex.token);  //guarda nombre de función
    lex.Next;    //Toma identificador
    //Busca la función
  ...
  end else if lex.tokType = tnIdentif then begin  //puede ser variable, constante, función
    GetOperandIdent(Op, ToSet);
    //Puede salir con error.
  end else if lex.tokType = tnBoolean then begin  //true o false
    {$IFDEF LogExpres} Result.txt:= cIn.tok; {$ENDIF}   //toma el texto
    TipDefecBoolean(Op, lex.token); //encuentra tipo y valor
    if HayError then exit;  //verifica
    lex.Next;    //Pasa al siguiente
   ...
   //Here we only know the operand is an array
    ...
    while not lex.atEof and (lex.token <> ']') do begin
      //Must be an item
  ...
  end else if lex.tokType = tnOperator then begin
    ...
    sino retrocedemos.}
    posAct := lex.GetCtxState;   //Esto puede ser pesado en términos de CPU
    oprTxt := lex.token;   //guarda el operador
    lex.Next; //pasa al siguiente
...
begin
  if lex.tokType <> tnOperator then exit(nil);
  //Hay un operador
  Result := Op.Typ.FindBinaryOperator(lex.token);
  if Result = nullOper then begin
    //No lo encontró, puede ser oeprador unario
    Result := Op.Typ.FindUnaryPostOperator(lex.token);
  end;
...
begin
  ...
  lex.SkipWhites;
  p := lex.GetCtxState;  //por si necesita volver
  ...
  while opr1<>nil do begin
    ...
    if opr1.OperationPost<>nil then begin  //Verifica si es operación Unaria
      ...
      p := lex.GetCtxState;  //actualiza por si necesita volver
      //Verifica operador
      opr1 := GetOperator(Op1);
      continue;
    end;
...
begin
  ...
  lex := TContexts.Create(xLex); //Crea lista de Contextos
  ejecProg := false;
...
begin
  unusedFuncs.Destroy;
  usedFuncs.Destroy;
  lex.Destroy; //Limpia lista de Contextos
  xLex.Free;
  listTypSys.Destroy;
  listFunSys.Destroy;
  TreeDirec.Destroy;
