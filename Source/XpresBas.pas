...
type
  ...
  public  //Manage of the Pre-error.
    ...
    ErrPosition: TSrcPos;    //***** -> PreErrPosit
  ...
  public
    ...
    procedure NewContextFromText(txt: string; arc0: String);
...
begin
  if nWarns = 1 then begin
    ...
    Result := IntToStr(nWarns) + ' warnings';
  end;
end;
...
begin
  if nErrors = 1 then begin
    ...
    Result := IntToStr(nErrors) + ' errors';
  end;
end;
...
procedure TContexts.NewContextFromText(txt: string; arc0: String);
...
function TContexts.GetMsgInfo(const txt: string): TMsgInfo;
...
function TContexts.GetMsgInfoE(const txt: string): TMsgInfo;
{Devuelve un objeto de mensaje (TMsgInfo) con el texto "txt" y la posición actual en el
...
begin
  if curCtx = nil then begin
    ...
    if curCtx.FixErrPos then begin
      ...
      Result.fname := ctxFile(curCtx.ErrPosition.idCtx);
      Result.Row := curCtx.ErrPosition.row;
...
function TContexts.GetMsgInfo(txt: string; const srcPos: TSrcPos): TMsgInfo;
...
function TContexts.GetMsgInfoE(const txt: string; const srcPos: TSrcPos): TMsgInfo;
{Devuelve un objeto de mensaje (TMsgInfo) con el texto "txt" y la posición "srcPos".
