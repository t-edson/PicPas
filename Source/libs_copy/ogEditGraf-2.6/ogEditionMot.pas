...
begin
  ...
  If EstPuntero = EP_SELECMULT then begin  //modo seleccionando multiples formas
      ...
      if objects.Count < 100 Then begin//sólo anima para pocos objetos
          for s In objects do begin
      ...
      CapturoEvento.MouseMove(xp, yp, selection.Count);
      Refresh;
  end else begin
...
begin
  If Shift = [] Then begin  //********************* Teclas normales ***********************
      ...
      If selection.Count = 0 Then begin  //si no hay objetos seleccionados
...
begin
  ...
  objects.Clear;    //limpia la lista de objetos
  EstPuntero := EP_NORMAL;
  ParaMover := false;
  CapturoEvento := nil;
...
var s: TObjGraf;
begin
  For s In objects do //no se explora "seleccion" porque se modifica con "s.Deselec"
...
begin
  ...
  objects := TlistObjGraf.Create(TRUE);   //crea lista con "posesión" de objetos
...
begin
  selection.Free;
  objects.Free;  //limpia lista y libera objetos apuntados
  v2d.Free;      //Libera
  //resatura eventos
  PBox.OnMouseUp:=nil;
