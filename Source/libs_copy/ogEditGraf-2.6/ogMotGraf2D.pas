...
TMotGraf = class
  ...
  procedure Text(x1, y1: Single; txt: String);
  procedure TextRect(x1, y1, x2, y2: Single; x0, y0: Single; const txt: string;
...
//funciones para texto
procedure TMotGraf.SetFont(Font: string);
//Permite definir el tipo de letra actual
begin
  if Font = '' then Canvas.Font.Name:= 'MS Sans Serif';
  //'Times New Roman'
end;
...
begin
   ...
   Canvas.Font.Underline := underline;
End;
procedure TMotGraf.Text(x1, y1: Single; txt: String);
//Escribe un texto
begin
...
End;
procedure TMotGraf.TextRect(x1,y1,x2,y2: Single; x0, y0: Single; const txt: string;
                       const Style: TTextStyle);
//Escribe un texto
var
  Arect: TRect;
...
//   Canvas.Font.Size := round(Canvas.Font.Size * Zoom);
   ...
   Canvas.TextRect(Arect, XPant(x0), YPant(y0), txt, Style);
//   Canvas.Font.Size := tmp;  //restaura
   Canvas.Brush.Style := bsSolid;  //devuelve estilo de fondo
End;
...
var
  Pnts: array of TPoint;    //arreglo de puntos a dibujar
begin
  SetLength(Pnts, 3);   //dimensiona
  //Llena arreglo
  Pnts[0].x := XPant(x1);         Pnts[0].y := YPant(y1+alto);
  Pnts[1].x := XPant(x1+ancho);   Pnts[1].y := YPant(y1+alto);
  Pnts[2].x := XPant(x1+ancho/2); Pnts[2].y := YPant(y1);
  Canvas.Polygon(Pnts);   //dibuja
end;
...
begin
  SetLength(Pnts, 3);   //dimensiona
  //Llena arreglo
  Pnts[0].x := XPant(x1);         Pnts[0].y := YPant(y1);
  Pnts[1].x := XPant(x1+ancho);   Pnts[1].y := YPant(y1);
  Pnts[2].x := XPant(x1+ancho/2); Pnts[2].y := YPant(y1+alto);
  Canvas.Polygon(Pnts);   //dibuja
end;
