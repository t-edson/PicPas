...
procedure TGenCodPic.GenerateROBdetComment;
...
procedure TGenCodPic.AssignRAM(out addr: word; regName: string; shared: boolean);
...
procedure TGenCodPic.AssignRAMbit(out addr: word; out bit: byte; regName: string; shared: boolean);
begin
...
function TGenCodPic.FreeStkRegisterByte: boolean;
...
procedure TGenCodPic.AssignRAMinByte(absAdd: integer;
...
procedure TGenCodPic.SetResultNull;
...
procedure TGenCodPic.SetResultConst(typ: TxpEleType);
...
procedure TGenCodPic.SetResultExpres(typ: TxpEleType; ChkRTState: boolean = true);
...
procedure TGenCodPic.SetResultVarRef(rVarBase: TxpEleVar);
begin
...
procedure TGenCodPic.SetResultExpRef(rVarBase: TxpEleVar; typ: TxpEleType; ChkRTState: boolean = true);
begin
  if ChkRTState then begin
    //Se pide verificar si se están suando los RT, para salvarlos en la pila.
    if RTstate<>nil then begin
...
procedure TGenCodPic.SetROBResultConst_bool(valBool: boolean);
begin
  GenerateROBdetComment;
  SetResultConst(typBool);
  res.valBool := valBool;
end;
procedure TGenCodPic.SetROBResultConst_bit(valBit: boolean);
begin
  GenerateROBdetComment;
  SetResultConst(typBit);
  res.valBool := valBit;
end;
procedure TGenCodPic.SetROBResultConst_byte(valByte: integer);
begin
  GenerateROBdetComment;
...
procedure TGenCodPic.SetROBResultConst_char(valByte: integer);
begin
  GenerateROBdetComment;
  SetResultConst(typChar);
  res.valInt := valByte;
end;
procedure TGenCodPic.SetROBResultConst_word(valWord: integer);
begin
  GenerateROBdetComment;
...
procedure TGenCodPic.SetROBResultConst_dword(valWord: Int64);
begin
  GenerateROBdetComment;
...
procedure TGenCodPic.SetROBResultVariab(rVar: TxpEleVar; Inverted: boolean);
begin
  GenerateROBdetComment;
  SetResultVariab(rVar, Inverted);
end;
//Fija el resultado de ROP como expresión
procedure TGenCodPic.SetROBResultExpres_bit(Opt: TxpOperation; Inverted: boolean);
...
procedure TGenCodPic.SetROBResultExpres_bool(Opt: TxpOperation; Inverted: boolean);
...
procedure TGenCodPic.SetROBResultExpres_byte(Opt: TxpOperation);
...
procedure TGenCodPic.SetROBResultExpres_char(Opt: TxpOperation);
...
procedure TGenCodPic.SetROBResultExpres_word(Opt: TxpOperation);
...
procedure TGenCodPic.SetROBResultExpres_dword(Opt: TxpOperation);
{Define el resultado como una expresión de tipo Word, y se asegura de reservar los
registros H,W, para devolver la salida.}
begin
  GenerateROBdetComment;
...
//Fija el resultado de ROU
procedure TGenCodPic.SetROUResultConst_bit(valBit: boolean);
begin
  GenerateROUdetComment;
  SetResultConst(typBit);
  res.valBool := valBit;
end;
procedure TGenCodPic.SetROUResultConst_byte(valByte: integer);
begin
  GenerateROUdetComment;
  if not ValidateByteRange(valByte) then
    exit;  //Error de rango
...
procedure TGenCodPic.SetROUResultVariab(rVar: TxpEleVar; Inverted: boolean);
begin
  GenerateROUdetComment;
  SetResultVariab(rVar, Inverted);
end;
procedure TGenCodPic.SetROUResultVarRef(rVarBase: TxpEleVar);
{Fija el resultado como una referencia de tipo stVarRefVar}
begin
  GenerateROUdetComment;
  SetResultVarRef(rVarBase);
end;
procedure TGenCodPic.SetROUResultExpres_bit(Inverted: boolean);
{Define el resultado como una expresión de tipo Bit, y se asegura de reservar el registro
...
procedure TGenCodPic.SetROUResultExpRef(rVarBase: TxpEleVar; typ: TxpEleType);
...
procedure TGenCodPic.ChangeResultCharToByte;
begin

end;
function TGenCodPic.ChangePointerToExpres(var ope: TOperand): boolean;
...
procedure TGenCodPic._MOVWF(const f: byte);
begin
  pic.flash[pic.iFlash].curBnk := CurrBank;
  pic.codAsmF(MOVWF, f);
  lastOpCode := _Inval;
...
procedure TGenCodPic.kANDWF(const f: TPicRegister; d: TPIC16destin); inline;
begin
  GenCodBank(f.addr);
  pic.flash[pic.iFlash].curBnk := CurrBank;
...
procedure TGenCodPic.kCLRF(const f: TPicRegister); inline;
begin
  GenCodBank(f.addr);
  pic.flash[pic.iFlash].curBnk := CurrBank;
...
procedure TGenCodPic.kINCF(const f: TPicRegister; d: TPIC16destin); inline;
begin
  GenCodBank(f.addr);
  pic.flash[pic.iFlash].curBnk := CurrBank;
...
procedure TGenCodPic.kMOVF(const f: TPicRegister; d: TPIC16destin);
begin
  GenCodBank(f.addr);
  pic.flash[pic.iFlash].curBnk := CurrBank;
...
procedure TGenCodPic.kMOVWF(const f: TPicRegister); inline;
begin
  GenCodBank(f.addr);
  pic.flash[pic.iFlash].curBnk := CurrBank;
...
procedure TGenCodPic.kRLF(const f: TPicRegister; d: TPIC16destin); inline;
begin
  GenCodBank(f.addr);
  pic.flash[pic.iFlash].curBnk := CurrBank;
...
procedure TGenCodPic.kRRF(const f: TPicRegister; d: TPIC16destin); inline;
begin
  GenCodBank(f.addr);
  pic.flash[pic.iFlash].curBnk := CurrBank;
...
procedure TGenCodPic.kSWAPF(const f: TPicRegister; d: TPIC16destin); inline;
begin
  GenCodBank(f.addr);
  pic.flash[pic.iFlash].curBnk := CurrBank;
...
procedure TGenCodPic.kBCF(const f: TPicRegisterBit);
begin
  GenCodBank(f.addr);
  pic.flash[pic.iFlash].curBnk := CurrBank;
...
procedure TGenCodPic.kBSF(const f: TPicRegisterBit);
begin
  GenCodBank(f.addr);
  pic.flash[pic.iFlash].curBnk := CurrBank;
...
procedure TGenCodPic.kBTFSC(const f: TPicRegisterBit);
begin
  GenCodBank(f.addr);
  pic.flash[pic.iFlash].curBnk := CurrBank;
...
procedure TGenCodPic.kBTFSS(const f: TPicRegisterBit);
begin
  GenCodBank(f.addr);
  pic.flash[pic.iFlash].curBnk := CurrBank;
...
procedure TGenCodPic.kXORWF(const f: TPicRegister; d: TPIC16destin); inline;
begin
  GenCodBank(f.addr);
  pic.flash[pic.iFlash].curBnk := CurrBank;
