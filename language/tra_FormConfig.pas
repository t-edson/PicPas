Caption              := Trans('Settings'             , 'Configuración', '','Einstellungen');
//General settings
tabGeneral.Caption   := Trans('General'              , 'General', '','Allgemein');
RadioGroup1.Caption  := Trans('Toolbar'              , 'Barra de herramientas', '','Werkzeugleiste');
...
//Assembler settings
tabEnsamb.Caption    := Trans('Assembler'            , 'Ensamblador', '','Assembler');
chkIncHeadMpu.Caption:= Trans('Include MPU &Header'  , 'Incluir &Encabezado de MPU', '','MPU &Kopfzeilen einfügen');
chkIncDecVar.Caption := Trans('Include &Variables declaration', 'Incluir Declaración de variables', '','Variablendeklaration einfügen');
RadioGroup2.Caption  := Trans('Style'                , 'Estilo', '','Stil');
chkExcUnus.Caption   := Trans('Exclude unused'       , 'Excluir no usadas', '','Unbenutzte ausschließen');
...
//Output Settings
tabOutput.Caption    := Trans('Output'               , 'Salida', '','Ausgabe');
