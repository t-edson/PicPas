...
lblBackground.Caption := Trans('<< No files >>', '<< Sin archivos >>', 'Mana khipukunayuq', '<< Keine Dateien >>','<< Немає файлів >>','<< Нет файлов >>');
mnNewTab.Caption      := Trans('New', 'Nuevo','Musuq','Neu','Новий','Новый');
mnCloseTab.Caption    := Trans('Close', 'Cerrar','Wisqay','Schließen','Закрити','Закрыть');
mnCloseAll.Caption    := Trans('Close All', 'Cerrar todos','Llapanta wisqay','Alle Schliessen','Закрити все','Закрыть всё');
mnCloseOthers.Caption := Trans('Close &others', 'Cerrar &otros','','','Закрити інші','Закрыть все другие');
if mnRecents<>nil then mnRecents.Caption:= trans('&Recents' ,'&Recientes','','&Letzte','Нещодавні','Недавние');
